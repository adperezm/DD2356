...
int main(int argc, char *argv[]){
    ...
    MPI_Sendrecv(&f[nxn_loc-3], 1, MPI_DOUBLE, (rank+1), 0, &f[0], 1, MPI_DOUBLE, (size-1), MPI_ANY_TAG, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    
    // Keep into consideration periodicity for rank 0 and size-1
    } else if (rank==size-1){
    
    //send left recieve right
    MPI_Sendrecv(&f[2], 1, MPI_DOUBLE, rank-1, 0, &f[nxn_loc-1], 1, MPI_DOUBLE, 0, MPI_ANY_TAG, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    //send right recieve left
    MPI_Sendrecv(&f[nxn_loc-3], 1, MPI_DOUBLE, 0, 0, &f[0], 1, MPI_DOUBLE, (rank-1), MPI_ANY_TAG, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    
    // Communicate normally for every other rank
    } else {
    
    //send left recieve right
    MPI_Sendrecv(&f[2], 1, MPI_DOUBLE, rank-1, 0, &f[nxn_loc-1], 1, MPI_DOUBLE, rank+1, MPI_ANY_TAG, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    //send right recieve left
    MPI_Sendrecv(&f[nxn_loc-3], 1, MPI_DOUBLE, rank+1, 0, &f[0], 1, MPI_DOUBLE, (rank-1), MPI_ANY_TAG, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    ...
    // Change the script to be able to print the data from the rank given in command line
    ...
    { 
      ...
      // Print f values
      int pp=atoi(argv[1]);
      if (rank==pp){ // print only rank 0 for convenience
