...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  // Print a header
  printf("====================================================================\n");
  printf("Performance of simple solver\n");
  printf("====================================================================\n");
  ...
  n_operations=dbln_steps*(  (dbln*((dbln-1)*18))   + (dbln*10)    );
    
  
  //Number of operations --- Assume 1 operation per cycle
  data_moved_size=dbln_steps*(  (dbln*((dbln-1)*16*sizeof(double)))   + (dbln*14*sizeof(double))    );
  
  printf("Performance Model predictions:\n");
  printf("Not considering data movement: %11.2f s \n", n_operations/c);
  printf("Considering data movement: %11.2f s \n", n_operations/c+data_moved_size*(1/bw));
