    ...
    // to compile with gcc
    // gcc -Wall -O2 -fopenmp -o DFTW_omp DFTW_omp_opt.c -lm 
    // written by stef
	...
	int main(int argc, char* argv[]){
	  ...
	  // Allocate array for output local vector
	  int mt=omp_get_max_threads();
  	  typedef double vect_t[mt];
  	  vect_t *Xr_loc,*Xi_loc;
  	  Xr_loc= malloc(N*sizeof(vect_t));
  	  Xi_loc= malloc(N*sizeof(vect_t));
  	  Xr_loc= memset(Xr_loc, 0, N*sizeof(vect_t));
  	  Xi_loc= memset(Xi_loc, 0, N*sizeof(vect_t));
	  ...
	  // DFT
	  int idft = 1;
	  DFT(idft,xr,xi,Xr_o,Xi_o,Xr_loc,Xi_loc,N);
	  // IDFT
	  idft = -1;
	  DFT(idft,Xr_o,Xi_o,xr_check,xi_check,Xr_loc,Xi_loc,N);
	...
	int DFT(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o,vect_t *X_r_loc,vect_t *X_i_loc, int N){
	  ...
	  for (int k=0 ; k<N ; k++)
	    {
	        ...
	        for (int n=0 ; n<N ; n++)  {
	        	// Real part of X[k]
	            Xr_loc[id,k] += xr[n] * cos(n * k * PI2 / N) + idft*xi[n]*sin(n * k * PI2 / N);
	            // Imaginary part of X[k]
	            Xi_loc[id,k] += -idft*xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
