    ...
    // to compile with gcc
    // gcc -Wall -O2 -fopenmp -o DFTW_optOmp.c DFTW_optOmp.c -lm 
    // written by stef
	...
	int DFT(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o, int N){
	  ...
	  double* Xr_o_priv=Xr_o;
	  double* Xi_o_priv=Xi_o;
	  ...
	  for (int k=0 ; k<N ; k++)
	    {
	        for (int n=0 ; n<N ; n++)  {
	            ...
	            Xr_o_priv[k] += xr[n] * cos(n * k * PI2 / N) + idft*xi[n]*sin(n * k * PI2 / N);
	            // Imaginary part of X[k]
	            //Xi_o[k] += -idft*xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
	            Xi_o_priv[k] += -idft*xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
