...
void laxf_scheme_2d(double *Q, double **ffx, double **ffy, double **nFx, double **nFy,
  ...
  for (i = 1; i < n; i++) {  //n is y direction... I want to devide the problem in columns and solve it
    double** ffxx=ffx;
    double** nnFx=nFx;
    fx(Q, ffxx, m, n, i);
    
    for (j = 1; j < m; j++)  //m is x direction
      for (k = 0; k < cell_size;  k++) 
	nnFx[k][j] = 0.5 * ((ffxx[k][j-1] + ffxx[k][j]) -
			   dx/dt * (Q(k, j, i) - Q(k, j-1, i)));  //Q is different for each thread
    for (j = 1; j < m-1; j++)
      for (k = 0; k < cell_size;  k++) 
	Q(k, j, i) = Q(k, j, i)  - dt/dx * ((nnFx[k][j+1] - nnFx[k][j]));
  }
